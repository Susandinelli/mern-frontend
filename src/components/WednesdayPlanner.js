...
export function WednesdayPlanner() {
  ...
  async function onSubmit(e) {
    ...
    await fetch("http://localhost:5000/wednesday/add", {
      method: "POST",
      headers: {
  ...
  async function getTask() {
    let { data } = await axios.get("http://localhost:5000/wednesday");
  ...
  async function onDelete(_id) {
    try {
      await axios.delete(`http://localhost:5000/wednesday/${_id}`);
    } catch (err) {
