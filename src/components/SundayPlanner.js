...
export function SundayPlanner() {
  ...
  async function onSubmit(e) {
    ...
    await fetch("https://planner-backend-664m.onrender.com/sunday/add", {
      method: "POST",
      headers: {
  ...
  async function getTask() {
    let { data } = await axios.get("https://planner-backend-664m.onrender.com/sunday");
  ...
  async function onDelete(_id) {
    try {
      await axios.delete(`https://planner-backend-664m.onrender.com/sunday/${_id}`);
    } catch (err) {
  ...
  return (
    <>
      ...
      <div class="flex justify-center align-middle text-olive-700 dark:text-olive-400 dark:bg-black">
        <div class="flex-col m-2 w-1/4 p-2 rounded-full bg-gradient-to-r hover:bg-gradient-to-t  from-olive-700 to-cocoa-700 via-navy-700 hover:from-olive-700 hover:to-cocoa-700 hover:via-navy-700">
